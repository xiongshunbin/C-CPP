...
bool SqlDao::isExist(QString loginName)
{
    QSqlQuery sql(m_db);
...
{
    QSqlQuery sql(m_db);
    QString strSql = QString("select u_password from sysuser where u_loginName = '%1'").arg(loginName);
    sql.exec(strSql);
