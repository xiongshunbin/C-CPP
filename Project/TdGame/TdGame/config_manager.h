...
public:
	...
	{
		double speed = 3;						// 移动速度
		double normal_attack_interval = 0.5;	// 普攻的冷却时间
		double normal_attack_damage = 0;		// 普攻伤害
		double skill_interval = 10;				// 技能的冷却时间
	...
	{
		double interval[10] = { 1 };			// 塔防放置的冷却时间
		double damage[10] = { 25 };				// 攻击力
		double view_range[10] = { 5 };			// 攻击范围
		double cost[10] = { 50 };				// 放置塔防所需花费
		double upgrade_cost[9] = { 75 };		// 升级塔防所需花费
	};
	...
	struct EnemyTemplate
	{
		double hp = 100;						// 生命值
		double speed = 1;						// 移动速度
		double damage = 1;						// 对房屋的攻击力
		double reward_ratio = 0.5;				// 消灭之后爆金币的概率
		double recover_interval = 10;			// 恢复生命值的冷却时间
		double recover_range = 0;				// 恢复生命值的范围
		double recover_intensity = 25;			// 恢复的生命值
	};
public:
	Map map;
	std::vector<Wave> wave_list;

	int level_archer = 0;		// 弓箭手的等级
	int level_axeman = 0;		// 斧头兵的等级
	int level_gunner = 0;		// 炮手的等级
	...
	const double num_initial_hp = 10;		// 房屋生命值
	const double num_initial_coin = 100;	// 金币数
	const double num_coin_per_prop = 10;	// 每次拾取金币获得金币数
...
public:
	...
	bool load_level_config(const std::string& path)
	{
		...
		if (!json_root)			return false;		// json解析失败
