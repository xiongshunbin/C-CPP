	...
	void on_update(double delta)
	{
		...
		if (is_releasing_flash)
		{
			...
			EnemyManager::EnemyList& enemy_list
				= const_cast<EnemyManager::EnemyList&>(EnemyManager::instance()->get_enemy_list());
			for (Enemy* enemy : enemy_list)
			{
				...
				if (enemy_position.x >= rect_hitbox_flash.x
					&& enemy_position.x <= rect_hitbox_flash.x + rect_hitbox_flash.w
					&& enemy_position.y >= rect_hitbox_flash.y
					&& enemy_position.y <= rect_hitbox_flash.y + rect_hitbox_flash.h)
				{
		...
		if (is_releasing_impact)
		{
			...
			for (Enemy* enemy : enemy_list)
			{
				...
				const Vector2& enemy_position = enemy->get_position();
				if (enemy_position.x >= rect_hitbox_impact.x
					&& enemy_position.x <= rect_hitbox_impact.x + rect_hitbox_impact.w
					&& enemy_position.y >= rect_hitbox_impact.y
					&& enemy_position.y <= rect_hitbox_impact.y + rect_hitbox_impact.h)
				{
...
protected:
	PlayerManager()
	{
		...
		const SDL_Rect& rect_map = ConfigManager::instance()->rect_tile_map;
		position.x = rect_map.x + static_cast<double>(rect_map.w) / 2;
		position.y = rect_map.y + static_cast<double>(rect_map.h) / 2;
...
private:
	void on_release_flash()
	{
		...
		case Facing::Left:
			...
			rect_hitbox_flash.y = (int)(position.y - static_cast<double>(68 / 2));
		...
		case Facing::Up:
			...
			rect_hitbox_flash.x = (int)(position.x - static_cast<double>(68 / 2));
			rect_hitbox_flash.y = (int)(position.y - size.y / 2 - 300);
			rect_hitbox_flash.w = 68, rect_hitbox_flash.h = 300;
			break;
		case Facing::Down:
			anim_effect_flash_current = &anim_effect_flash_down;
			rect_hitbox_flash.x = (int)(position.x - static_cast<double>(68 / 2));
			rect_hitbox_flash.y = (int)(position.y + size.y / 2);
			rect_hitbox_flash.w = 68, rect_hitbox_flash.h = 300;
			break;
		}
	...
	void on_release_impact()
	{
		...
		case Facing::Left:
			...
			rect_hitbox_impact.y = (int)(position.y - static_cast<double>(140 / 2));
		...
		case Facing::Up:
			...
			rect_hitbox_impact.x = (int)(position.x - static_cast<double>(140 / 2));
			rect_hitbox_impact.y = (int)(position.y - size.y / 2 - 60);
